...
class DBPlacetypes extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        inpRaces.value = settings.story.db.placetypes.join(',');
        ...
        this.elements.btnAdd.addEventListener('click', (evt) => {
            ...
            // inpRaces.value.replaceAll(' ', ',');
            inpRaces.value.replaceAll('.', ',');
            settings.story.db.placetypes = inpRaces.value.split(',');
...
customElements.define('db-placetypes', DBPlacetypes);